...
def visualize_mz_embedding(outputs_final, file_list, interest_mz, output_path=None):
    ...
    # Find ions within the m/z tolerance
    ppm = 10
    ...
    within_tolerance = (
        np.abs(mz_values - interest_mz) <= tolerance
    )
    
    # One point per sample
    selected_indices = []
    
    for sample_id in np.unique(sample_ids_array[within_tolerance]):
    
        candidate_indices = np.where(
            within_tolerance & (sample_ids_array == sample_id)
        )[0]
    
        # Select the ion whose m/z is closest to interest_mz
        best_idx = candidate_indices[
            np.argmin(
                np.abs(mz_values[candidate_indices] - interest_mz)
            )
        ]
    ...
    highlighted_points = pca_results[selected_indices]
    highlighted_ids = sample_ids_array[selected_indices]
    highlighted_mz = mz_values[selected_indices]
